...
| 누락된 Hreflang 감지 | 콘텐츠(온사이트) | hreflang 속성이 누락된 플래그 페이지입니다. 언어/지역별로 영향을 받는 URL과 예상 범위를 제공합니다. | 올바른 현지화 버전을 표시하기 위해 hreflang 태그를 구현합니다. |
| 누락된 정식 검색 | 콘텐츠(온사이트) | 정식 태그가 없거나 충돌하는 태그가 있는 페이지를 검색합니다. 영향을 받는 URL 및 중복 요소를 나열합니다. | 각 페이지의 기본 버전을 가리키는 공식 태그를 추가합니다. 변형 사이에서도 일관되게 사용할 수 있습니다. |
