...
>[!NOTE]
>
>Edge에서 최적화 규칙을 추가한 후 속성 활성화에 실패하는 경우, 규칙이 기본 규칙과 다른 원본 서버 SSL 확인 모드를 사용하는지 여부를 확인합니다. 이 경우 기본 규칙과 일치하도록 Edge에서 최적화 규칙을 업데이트합니다. 예를 들어 기본 규칙에서 **플랫폼 설정**&#x200B;을 사용하는 경우 여기에서도 **플랫폼 설정**&#x200B;을 사용하십시오. 필요한 설정을 사용할 수 없는 경우 Akamai 지원 센터에 문의하십시오.
...
>[!IMPORTANT]
>
>이 단계의 XML 스니펫에는 **고급** 동작이 필요합니다. 일부 Akamai 환경에서는 이 비헤이비어를 셀프서비스 편집에서 사용할 수 없습니다. **고급** 옵션이 표시되지 않는 경우 Akamai 계정 팀이나 Akamai 지원 팀에 연락하여 필요한 구성을 활성화하십시오.
...
>
>또한 동일한 요청에 대해 원본, 캐싱 동작 또는 캐시 ID를 변경하는 나중에 일치하는 규칙에 의해 **Edge 라우팅에서 최적화** 규칙이 재정의되지 않도록 합니다. 다른 일치 규칙이 이러한 동작을 재설정하면 Edge 라우팅 또는 캐싱에서 최적화가 예상대로 작동하지 않을 수 있습니다.
