...
다음 Akamai 속성 관리자 규칙은 아젠틱 HTML 페이지 트래픽을 Edge 최적화로 라우팅합니다. 구성에는 다음 단계가 포함됩니다.

**1. 라우팅 조건 설정(사용자 에이전트 및 HTML 트래픽 일치)**
...
>[!NOTE]
>
>Edge에서 최적화 라우팅 규칙을 에이전트 HTML 페이지 트래픽에만 적용합니다. 일반적인 설정은 확장명이 없는 페이지 URL에 대해 `html` 및 `EMPTY_STRING`을(를) 일치시키기 위해 **파일 확장명**&#x200B;과 같은 요청 측 기준을 사용하는 것입니다. 사이트가 다른 URL 패턴에서 HTML을 제공하거나 API 끝점과 같은 무제한 비페이지 경로를 포함하는 경우 추가 경로 기반 기준으로 규칙을 세분화하십시오.
...
`x-edgeoptimize-config`에서 (으)로 `LLMCLIENT=TRUE;`
...
LLM Optimizer에서 스테이징 호스트 이름과 스테이징 API 키를 사용하는 경우, 규칙의 **스테이징** 키를 사용하여 **스테이징** Akamai 속성에 동일한 라우팅 패턴을 배포하십시오. 그런 다음 스테이징 호스트에서 보트 트래픽을 확인합니다.
