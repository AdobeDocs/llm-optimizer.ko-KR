...
2. **헤더** 허용 목록에 `x-edgeoptimize-config` 및 `x-edgeoptimize-url`을 추가합니다.

   * 드롭다운에서 **다음 헤더 포함**&#x200B;을 선택합니다.
   * `x-edgeoptimize-config` 및 `x-edgeoptimize-url`을 추가합니다.
...
3. **오브젝트 캐싱** 설정을 확인합니다.
   ...
   * **사용자 지정**&#x200B;으로 설정하는 경우 **최소 TTL**&#x200B;을 `0`으로 설정하는 것이 좋습니다. 현재 최소 TTL이 이미 매우 짧은 경우 변경할 필요가 없습니다.
...
3. 1부에 언급된 모든 설정을 다음과 같이 수정하여 복제합니다.
   ...
   * **최소 TTL**&#x200B;을 `0`으로 설정하는 것이 좋습니다. 현재 최소 TTL이 이미 매우 짧은 경우 변경할 필요가 없습니다.

   ![캐시 정책 TTL 설정](/help/assets/optimize-at-edge/cloudfront-cache-policy-ttl.png)

   * **캐시 키 설정** > **헤더**&#x200B;에서 관리 정책에 있는 모든 내용을 포함하고 `x-edgeoptimize-config` 및 `x-edgeoptimize-url`을 추가합니다.
...
2. 3단계(시나리오 C)에서 새 캐시 정책을 만든 경우 **캐시 정책**&#x200B;을 `edgeoptimize-cache`로 설정합니다.
