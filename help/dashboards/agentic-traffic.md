...
에이전트 트래픽 대시보드는 AI 에이전트(웹 크롤러 및 챗봇)가 사이트와 상호 작용하는 방법을 보여 줍니다. 이 보기를 사용하여 총 요청 수 및 일반 성능 관련 지표를 추적할 수 있습니다. 시장, 카테고리, 페이지 및 에이전트에 대한 트래픽 분포를 확인할 수도 있습니다. 이 대시보드에서 사용하는 데이터는 CDN 로그에서 가져온 것이므로 지표를 표시하려면 **CDN 로그 전달**&#x200B;을(를) 구성해야 합니다. 표시된 데이터를 구체화하는 데 도움이 되는 맞춤형 필터도 있습니다.
...
LLM Optimizer은 CDN 로그에서 필드의 하위 집합을 처리합니다. 원시 로그 필드 이름은 CDN 공급자에 따라 다르지만 다음과 같이 표준화되고 표시됩니다.
...
이러한 정규화된 필드는 무의미한 보기를 통해 노출된다. [참조 트래픽](/help/dashboards/referral-traffic.md) 대시보드에서 CDN 로그가 페이지 히트 지표를 표시하는 데 사용됩니다. CDN 로그 수집 또는 후속 데이터 처리 단계에서는 PII(개인 식별 정보)가 처리되거나 저장되지 않습니다.
...
* **에이전트 유형** - 사이트와 상호 작용한 AI 에이전트 유형별로 필터링합니다. 웹 크롤러, 챗봇 또는 모든 에이전트 사이를 필터링할 수 있습니다.
...
상단 및 하단 무버 보기는 콘텐츠에 액세스하는 AI 시스템의 방문 또는 히트와 같은 아젠틱 트래픽에서 주 단위로 가장 큰 변화가 발생한 URL을 강조 표시합니다. **상위 이동자**&#x200B;는 페이지 가시성 또는 참여도를 보여 주고, **하위 이동자**&#x200B;은(는) 가장 빠른 감소로 URL을 보여 줍니다. 이를 통해 어떤 콘텐츠가 상승하고 있는지, 주의가 필요하고 AI 기반 검색 패턴이 어디로 이동하는지 빠르게 파악할 수 있습니다.

![상위 및 하위 이동자](/help/dashboards/assets/movers.png)

## 사용자 에이전트 및 URL 성능 분석 {#user-url-performance}

사용자 에이전트 및 URL 성능 분석 보기는 웹 크롤러 및 챗봇이 사이트와 상호 작용하는 방법에 대한 추가 데이터 분류를 제공합니다. 자세한 설명은 아래 탭을 클릭하십시오.
...
사용자 에이전트 분석 테이블은 페이지 유형 및 에이전트 유형(예: 웹 크롤러 대 챗봇)별 트래픽 분류를 제공합니다. 크롤링 이렇게 하면 어떤 AI 에이전트가 사이트의 어떤 부분인지 쉽게 이해할 수 있습니다. 여기에는 다음 카테고리가 포함됩니다.

* **페이지 유형** - 페이지 유형입니다.
* 크롤링 **에이전트 유형** - AI 에이전트 페이지의 웹 크롤러 또는 챗봇입니다.
...
URL 성능 분석 표에는 개별 URL에 대한 세부 보기가 표시됩니다. 여기에는 히트 수, 고유 에이전트, 최상위 에이전트, 성공률 및 범주가 포함됩니다. 이렇게 하면 가치가 높은 페이지를 식별하고, 크롤링 간격을 감지하고, AI 엔진을 위한 콘텐츠를 최적화할 수 있습니다. URL은 트래픽 볼륨별로 순위가 매겨집니다. 이 표에는 다음과 같은 범주가 포함되어 있습니다.
...
URL 세부 정보 보기는 페이지 성능을 인용하는 빈도, 언급된 AI 응답 감정, 페이지가 표시되는 주제 및 프롬프트, 시간 경과에 따른 에이전트 및 참조 트래픽 트렌드를 보여 주는 전체적인 이해를 제공합니다.

>[!ENDTABS]

두 테이블 모두에서 **내보내기** 옵션을 사용하여 테이블 `.csv`을(를) 다운로드하고 팀과 인사이트를 공유하거나 경영진 보고에 테이블을 포함할 수 있습니다.
