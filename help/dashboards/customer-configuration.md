...
**프롬프트** 탭에서 프롬프트를 검토하고 관리하고 사용자 지정할 수 있습니다. [브랜드 존재감 분석](/help/dashboards/brand-presence.md) .csv를 업로드할 수 있으며 이 목록은 해당 분석의 프롬프트와 토픽 또는 Adobe에서 만든 [프롬프트 라이브러리 다운로드](/help/overview/best-practices-topics-prompts.md)로 채워집니다. 필요에 따라 토픽 및 관련 프롬프트를 삭제, 수정 및 추가할 수도 있습니다.
...
1. **고객 구성** 대시보드(클래식 탐색) 또는 **브랜드 관리**(브랜드 중심 경험)를 연 다음 Google 검색 콘솔 통합(브랜드 중심 경험의 GSC 태그)로 이동합니다.
