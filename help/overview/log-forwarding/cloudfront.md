...
description: LLM Optimizer의 에이전트 트래픽 데이터 수집을 위해 CloudFront에서 Adobe의 S3 버킷으로 CDN 로그를 전달하는 방법을 알아봅니다.
...
이 페이지에서는 에이전트 트래픽 데이터 수집을 위해 CloudFront에서 Adobe의 S3 버킷으로 CDN 로그를 전달하는 방법을 설명합니다. LLM Optimizer CDN 구성 페이지를 사용하여 LLM Optimizer에 온보딩합니다. 온보딩 프로세스가 완료되면 이 페이지에 제공된 단계에 따라 CloudFront 대시보드 콘솔에서 로그 전달을 구성합니다.
...
1. **CloudFront(BYOCDN)**&#x200B;을(를) 선택합니다.

   ![CloudFront 선택](/help/overview/assets/log-forwarding/cloudfront/cloudfront-select.png)

1. **온보드**&#x200B;를 클릭합니다.

   ![온보드 단추](/help/overview/assets/log-forwarding/common/onboard-button.png)

## 2단계: 표준 로깅 활성화(CloudFront 콘솔) {#step-2}

표준 로깅을 사용하려면 [AWS 관리 콘솔](https://aws.amazon.com/console/)에서 다음을 수행합니다.

1. [CloudFront 콘솔에 액세스](https://console.aws.amazon.com/cloudfront/v4/home) 및 [기존 배포를 업데이트](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/HowToUpdateDistribution.html#HowToUpdateDistributionProcedure)합니다.

1. **로깅** 탭을 엽니다.

1. **추가**&#x200B;를 선택한 다음 로그를 수신할 서비스를 선택하십시오(이 경우 **Amazon S3**).

1. **대상**&#x200B;에 대해 리소스를 선택하거나 만드십시오. **버킷 이름**&#x200B;을 입력하세요. LLM Optimizer CDN 구성 페이지에서 값을 복사할 수 있습니다.
...
1. **추가 설정** 구성:
   ...
   - **분할** — LLM Optimizer 구성 페이지에서 **경로 접미사**&#x200B;를 복사합니다.

     ![CloudFront 파티션 나누기](/help/overview/assets/log-forwarding/cloudfront/cloudfront-partitioning.png)
...
1. **로그** 페이지에서 배포 옆에 **사용**&#x200B;이 표시되는지 확인하십시오.
...
예를 들어 원본 계정 `111111111111`이(가) 대상 계정 `222222222222`의 S3 버킷에 로그를 보냅니다. [AWS 명령줄 인터페이스](https://aws.amazon.com/cli/)를 사용할 수 있습니다.

>[!NOTE]
>
>아래 명령에서 배달 대상 ARN 값(`arn:aws:logs:us-east-1:222222222222:delivery-destination:cloudfront-delivery-destination`)을 LLM Optimizer 구성 페이지의 **배달 대상 ARN** 값으로 바꿉니다.
...
1. **게재를 만듭니다** - 원본을 대상에 연결합니다. &quot;대상 계정 구성&quot; 단계에서 대상 ARN을 사용합니다.
...
1. **확인:**
   ...
   - **소스** 계정: CloudFront 콘솔 > 배포 > **로깅** 탭에서. **Type**&#x200B;에서 S3 계정 간 로그 전달이 표시됩니다.
   - **대상** 계정: S3 콘솔 > 버킷. 접두사(예: `MyLogPrefix`)와 해당 폴더에 로그가 표시됩니다.
