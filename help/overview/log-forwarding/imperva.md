---
title: 로그 전달 - Imperva
description: LLM Optimizer의 에이전트 트래픽 데이터 수집을 위해 CDN 로그를 Imperva에서 Adobe의 S3 버킷으로 전달하는 방법을 알아봅니다.
...
이 안내서에서는 에이전트 트래픽 데이터 수집을 위해 CDN 로그를 Imperva에서 Adobe의 S3 버킷으로 전달하는 방법을 설명합니다. LLM Optimizer CDN 구성 페이지를 사용하여 LLM Optimizer에 온보딩합니다. 온보딩 프로세스가 완료되면 이 페이지에 제공된 단계에 따라 Imperva 웹 콘솔에서 로그 전달을 구성합니다.
...
[Imperva 콘솔](https://my.imperva.com)에서:
...
3. 연결 유형(로그 대상)으로 **Amazon S3 ARN**&#x200B;을(를) 선택하십시오.

4. 다음을 입력하십시오.

   | 필드 | 설명 | 메모 |
   |---|---|---|
   | **연결 이름** | 연결을 설명하는 이름(예: 프로덕션 S3 로그)입니다. 기본값 이름을 바꿀 수 있습니다. | |
...
5. **연결 테스트**&#x200B;를 클릭합니다. Imperva는 테스트 파일(실제 데이터 없음)이 지정된 폴더로 전송된 다음 전송이 완료되면 제거되는 전체 테스트를 실행합니다.

   - **사용 가능** — 저장소 세부 정보가 유효합니다. 이 연결을 사용하도록 로그를 구성할 수 있습니다.
   - **정의되지 않음** — 필요한 세부 정보가 없거나 테스트가 실패했습니다.

6. 구성을 저장하려면 **저장**&#x200B;을 클릭하세요.
...
7. 로그 옵션(로그 유형, 로그 수준, 형식, 압축) 및 **로그 수준**&#x200B;을 구성합니다. LLM Optimizer 구성 페이지에서 값을 가져올 수 있습니다.
   ...
   | 게재 방법 | ![게재 메서드](/help/overview/assets/log-forwarding/imperva/imperva-delivery-method.png) |
