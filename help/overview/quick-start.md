...
정확한 컨텐츠 및 기술 권장 사항을 전달하려면 Adobe LLM Optimizer에서 공개 페이지에 액세스해야 합니다. 이는 보안 내부 웹 크롤러(Spacecat/1.0 사용자 에이전트)을 통해 수행됩니다.
...
콘텐츠 가시성이 대시보드에 낮게 표시되면 웹 크롤러가 도메인에 액세스할 수 있는지 확인합니다. 제한된 액세스는 불완전한 색인화의 일반적인 원인입니다.
...
* 최적화 배포 기능은 Early Access에서 사용할 수 있습니다. 자세한 내용은 [Edge에서 최적화](https://experienceleague.adobe.com/ko/docs/llm-optimizer/using/resources/optimize-at-edge#frequently-asked-questions)를 참조하세요.
...
온보딩 단계 중에 작은 범주, 주제 및 프롬프트 세트가 생성됩니다. 이러한 프롬프트에 대한 브랜드 존재감 분석은 사이트가 온보딩된 직후에 사용할 수 있습니다.

<!--![Brand Presence Analysis](/help/overview/assets/bp-analysis.png)-->

또한 트래픽 분석을 위해 [CDN 로그 전달](#step-4)을 구성해야 합니다. LLM Optimizer은 AI 가시성을 높이기 위해 기회를 식별하고 처방적 권장 사항을 제공하기 위해 에이전시와 참조 트래픽의 브랜드 존재감 데이터와 통찰력이 필요합니다.
...
* [브랜드 존재감](/help/dashboards/brand-presence.md) 대시보드를 보고 다른 브랜드와 비교하여 가시성 점수를 보고 성과를 추적하세요.
