...
1. **고객 구성** 대시보드로 이동하여 LLM Optimizer에서 프롬프트 라이브러리 파일을 다운로드합니다.
2. 각 탭에서 브랜드 산업에 대해 제안된 **주제** 및 **프롬프트**&#x200B;를 검토하고 가장 관련성이 높은 옵션을 선택하십시오.
3. **고객 여정 단계 열**&#x200B;을(를) 검토하여 고객 라이프사이클 전반에 걸친 프롬프트 옵션을 확인합니다(예: 검색에서 보존으로 전환). funnel 프롬프트의 초기 단계/최상위에는 우선 순위가 높지만, 나중에 보존 상태를 홍보하고 고객 지원을 활성화하는 등의 옵션을 고려하십시오.
4. Adobe LLM Optimizer에 업로드하기 전에 목표 및 목표를 최대한 지원하도록 주제 또는 프롬프트를 수정합니다(예: 브랜드/제품 이름 추가, 브랜드 용어 추가). 프롬프트는 수동으로 LLMO에 추가하거나 제공된 *.csv* 템플릿을 사용하여 일괄 업로드할 수 있습니다.
